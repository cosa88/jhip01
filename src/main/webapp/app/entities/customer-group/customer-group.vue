<template>
  <div>
    <h2 id="page-heading" data-cy="CustomerGroupHeading">
      <span v-text="$t('jhip01App.customerGroup.home.title')" id="customer-group-heading">Customer Groups</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhip01App.customerGroup.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CustomerGroupCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhip01App.customerGroup.home.createLabel')"> Create a new Customer Group </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customerGroups && customerGroups.length === 0">
      <span v-text="$t('jhip01App.customerGroup.home.notFound')">No customerGroups found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="customerGroups">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('jhip01App.customerGroup.name')">Name</span></th>
            <th scope="row"><span v-text="$t('jhip01App.customerGroup.enName')">En Name</span></th>
            <th scope="row"><span v-text="$t('jhip01App.customerGroup.customer')">Customer</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="jhip01App.customerGroup.delete.question" data-cy="customerGroupDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-customerGroup-heading" v-text="$t('jhip01App.customerGroup.delete.question', { id: removeId })">
